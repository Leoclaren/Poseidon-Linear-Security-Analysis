...
def plot_diffusion(t=3, r_p=10, alpha=5, delta_val=4, save=None):
...
def plot_branch_numbers(t=3, save=None):
    ...
    for bar, bn in zip(bars, bns):
        ax.text(bar.get_x() + bar.get_width() / 2, bn + 0.05,
                str(bn), ha="center", va="bottom", fontsize=12, fontweight="bold")

    ax.set_ylabel("Branch number B(M)", fontsize=12)
    ax.set_title(f"Branch numbers  |  t={t}", fontsize=13)
    ax.set_ylim(0, max(bns) + 1.5)
    ax.legend(fontsize=10)
...
def plot_all(t=3, r_p=10, alpha=5, delta_val=4, save=None):
    ...
    names = list(MATRIX_NAMES.keys())
    bns   = [branch_number(MATRIX_NAMES[n]) for n in names]
    ax2.bar([n for n in names], bns, color=[COLORS[n] for n in names],
    ...
    ax3.bar(names, final_d, color=[COLORS[n] for n in names],
            edgecolor="black", linewidth=0.8)
    ...
    for n in names:
        ...
        colors_table.append(
            [COLORS[n], "white", "#fadbd8" if not stats[n]["is_mds"] else "#d5f5e3",
...
def main():
    ...
    parser.add_argument("--mode", choices=["diffusion", "branch", "stats", "all"],
    ...
    kwargs = dict(t=args.t, r_p=args.r_p, alpha=args.alpha, delta_val=args.delta, save=args.save)
